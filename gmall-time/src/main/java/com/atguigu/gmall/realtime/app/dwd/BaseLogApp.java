...
//        env.setStateBackend(new FsStateBackend("hdfs://hadoop101:8020/gmall/checkpoint/baselogApp"));
...
//        */
//        //按照mid进行分组，因为是按照mid来处理的
//        KeyedStream<JSONObject, String> midKeyedDS = jsonObjDS.keyBy(data -> data.getJSONObject("common").getString("mid"));
//
//
//        SingleOutputStreamOperator<Object> jsonDSWithFlag = midKeyedDS.map(new RichMapFunction<JSONObject, Object>() {
//            //定义该mid的访问状态
...
//
//            @Override
//            public Object map(JSONObject jsonObj) throws Exception {
//                //获取日志标记状态
...
//
//                    //如果状态不为空，并且状态日期和当前日期不同不相等，说明是老访客
...
//        });
//
//        //分流  分流  根据日志数据内容,将日志数据分为3类, 页面日志、启动日志和曝光日志。
...
//
//        SingleOutputStreamOperator<Object> pageDS = jsonDSWithFlag.process(
